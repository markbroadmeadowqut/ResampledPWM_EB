...
void initPWM() {
	...
	EPwm1Regs.DBCTL.bit.POLSEL = 2; // AHC dead-band mode
	EPwm1Regs.DBCTL.bit.OUT_MODE = 3; // AHC dead-band mode
	...
	EPwm2Regs.ETSEL.bit.SOCASEL=6; // Trigger SOCA when CTR=CMPB and timer is incrementing to begin with because ePWM2 starts counting up
	EPwm2Regs.CMPB = 3000;
	...
	EPwm3Regs.ETSEL.bit.SOCASEL=7; // Trigger SOCA when CTR=CMPB and timer is decrementing to begin with because ePWM3 starts counting down
	EPwm3Regs.CMPB = 2250;
