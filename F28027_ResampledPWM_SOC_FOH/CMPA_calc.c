...
volatile Uint16 prevprevSamplehalved = 0;
...
interrupt void isr_CMPA_calc(void)
{
	...
	if(swCTRDIR) {
		...
		ti=((swTBCTR-newSample+prevSample-prevprevSamplehalved)*FOH_SCALE)/calc1_rising;
		...
		swTBCTR = EPwm1Regs.TBCTR;
		ti=((swTBCTR-newSample+prevSample-prevprevSamplehalved)*FOH_SCALE)/calc1_falling;
		CMPA_value=swTBCTR-ti;
		EPwm1Regs.CMPA.half.CMPA=CMPA_value;
		//DEBUG
		GpioDataRegs.GPACLEAR.bit.GPIO29 = 1;
	...
	prevprevSamplehalved=prevSample/2;
	prevSample = AdcResult.ADCRESULT0;       // Push newSample back on to prevSample at this point in the interrupt so that the write to CMPA happens sooner
